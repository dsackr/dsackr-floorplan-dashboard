...
class FloorplanManagerPanel extends LitElement {
  ...
  _debouncedSave() {
    ...
    this._saveTimeout = setTimeout(() => this._saveConfig(), 500);
  }
  ...
  _updateEntity(entityId, updates) {
  ...
  _filteredEntities() {
    ...
    return entities.filter((entityId) => entityId.toLowerCase().includes(search));
  }
  ...
  _startDrag(ev, type, id) {
    ev.preventDefault();
    this._dragging = { type, id };
    const move = (moveEv) => this._onDrag(moveEv);
    const end = () => {
  ...
  _renderEntitiesList() {
    ...
    return this._filteredEntities().map((entityId) => {
          ...
          <div>
            <label class="small">
              <input
                ...
                @change=${(ev) => this._updateEntity(entityId, { include: ev.target.checked })}
              />
              include
            </label>
      ...
      return html`
          ...
          <div>
            <label class="small">
              <input
                type="checkbox"
                .checked=${config.include || false}
                @change=${(ev) => this._updateArea(area.id, { include: ev.target.checked })}
              />
              include
            </label>
        ...
        <div>
          ...
          <input
            ...
            max="50"
            .value=${config.r ?? 8}
            @input=${(ev) => this._updateArea(area.id, { r: Number(ev.target.value) })}
          />
        </div>
      `;
    });
        ...
        <div class="section">
          ...
          <div class="row">
            <input
              ...
              .value=${imageUrl}
              @input=${(ev) => this._setImageUrl(ev.target.value)}
            />
          </div>
          ...
          <div class="image-wrapper" @click=${this._handleImageClick}>
            ${imageUrl ? html`<img src=${imageUrl} alt="Floorplan" />` : html`<div class="small">Upload or enter an image URL.</div>`}
            ${Object.entries(areas).map(([areaId, area]) =>
              area.include && typeof area.x === "number" && typeof area.y === "number" && typeof area.r === "number"
                ? html`<div
                    ...
                    @pointerdown=${(ev) => this._startDrag(ev, "area", areaId)}
                  ></div>`
                : ""
            )}
            ${Object.entries(entities).map(([entityId, entity]) =>
              entity.include && typeof entity.x === "number" && typeof entity.y === "number"
                ? html`<div
                    ...
                    title=${entityId}
                    @pointerdown=${(ev) => this._startDrag(ev, "entity", entityId)}
                  ></div>`
                : ""
            )}
          </div>
